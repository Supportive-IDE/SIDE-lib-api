...
function _parseLine2(startFrom, varsWithTypeNames, moduleNames, classDefinitionInProgress) {
  ...
  for (var i = startFrom; i < _classPrivateFieldGet(this, _text).length; i++) {
    var currentCharCategory = _enums.Character.getCategory(_classPrivateFieldGet(this, _text).charCodeAt(i));
...
function _completeBlockComment2(delimiter, contentStart) {
  if (contentStart < _classPrivateFieldGet(this, _text).length) {
    ...
    var closeCommentIndex = cleanText.substring(contentStart).indexOf(delimiter);
    // Comment ends on this source line
...
function _processStringLiteral2(quoteChar, localIndex) {
  ...
  var contentStart = _classPrivateFieldGet(this, _stringLiteralDelimiter) === undefined ? localIndex + delimiter.length : localIndex;
  // First quote of closing delimiter
  var closeStringIndex = contentStart + cleanText.substring(contentStart).indexOf(delimiter);
  // String does not end on this source line
...
function _processIdentifierOrKeyword2(localIndex, varsWithTypeNames, moduleNames, classDefinitionInProgress) {
  ...
  var expression = (0, _ast.expressionFactory)(name, info, _classPrivateFieldGet(this, _currentLineNumber), _classPrivateFieldGet(this, _currentLineStartIndex) + localIndex, localIndex);
  // not in, is not
...
function _processMultiCharacterOperator2(localIndex) {
  ...
  if (operator === "*" && _classPrivateMethodGet(this, _hasExpressions, _hasExpressions2).call(this) && _classPrivateMethodGet(this, _getLastExpression, _getLastExpression2).call(this).is(_enums.ExpressionEntity.ImportKeyword)) {
    operatorInfo = new _utils.ExpressionInfo(_enums.ExpressionEntity.ImportAll, _enums.ExpressionCategory.OtherKeywords);
...
function _processSingleCharacter2(localIndex) {
  ...
  var info = (0, _utils.keywordLookup)(text);
  // OLD VERSION: set subscripted if [
...
var StatementProcessor = /*#__PURE__*/function () {
  ...
  _createClass(StatementProcessor, null, [{
      ...
      if (!statement.isBlank()) {
        ...
        if (lastStatement.isBlockStatement()) {
          ...
          if (!(firstExpOfLastStatement.isOneOf([_enums.ExpressionEntity.IfDefinitionStatement, _enums.ExpressionEntity.ElifDefinitionStatement]) && firstExpOfNewStatement.isOneOf([_enums.ExpressionEntity.ElseDefinitionStatement, _enums.ExpressionEntity.ElifDefinitionStatement]))) {
            var blockExpressions = lastStatement.getExpressions();
    ...
    /**
     * Helper method. Updates the defined in block for for loop variables
     * @param {StatementBlock} block 
     * @returns {StatementBlock}
     */

    /**
     * Adds imported entities to module tracking for the block
    ...
    value: function checkIfVariableExistsInParent(vInfo, block) {
      var parent = block.getScope().getParentBlock();
      // Check that vInfo first usage is not assigned or changed
    ...
    /**
     * Helper method to check if a definition keyword is being assigned as if it were a variable name.
     * @param {ExpressionNode} expressions 
     * @returns {Boolean} 
     */

    /**
     * Calls the appropriate statement processor for the type of definition and returns a definition statement
     * @param {ExpressionNode[]} expressions 
     * @returns {MultiPartExpressionNode}
     */

    /**
     * Determines whether to create a function definition statement or a method definition statement
     * @param {ExpressionNode[]} expressions 
     * @returns {FunctionDefinitionStatement | MethodDefinitionStatement }
     */

    //#endregion - definition statements

    //#region - groups

    /**
     * Processes all multipart expressions involving groups
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Finds all matched brackets in an expressions array
     * @param {ExpressionNode[]} expressions 
     * @returns {Number[][]} A 2D array of expression indices
     */

    /**
     * Creates multi-part expressions involving parentheses. Assumes that definition lines aren't included
     * @param {Number[]} brackets An array of length two that contains that indices of the open and close parentheses
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Creates multi-part expressions involving square brackets
     * @param {Number[]} brackets An array of length two that contains that indices of the open and close brackets
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */
  }, {
    ...
    /**
     * Creates multi-part expressions involving square braces
     * @param {Number[]} brackets An array of length two that contains that indices of the open and close braces
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Factory function to create a multipart expression.
     * @param {ExpressionNode[]} children The child nodes that make up the multipart expression
     * @param {ExpressionEntity} entity The entity of the multipart expression
     * @param {ExpressionNode[][]} elements Optional. The elements that make up a portion of the multipart expression e.g. the parameters in a function definition
     * @returns {MultiPartExpressionNode}
     * @throws Throws an error if the entity is not recognised.
     */
    ...
    //#region - calculations
    /**
     * Processes all multipart expressions involving calculations
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Combines any sequential string literals
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Gets the index of the calculation entity with the highest precedence. Returns
     * -1 if there are no calculation entities.
     * @param {ExpressionNode[]} expressions 
     * @returns {Number} The index of the calculation entity or -1 if no calculation entities are found
     */
    ...
    //#region - comparisons
    /**
     * Processes all multipart expressions involving comparisons
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Gets the index of the comparison entity. Returns -1 if there are no comparison entities.
     * @param {ExpressionNode[]} expressions 
     * @returns {Number} The index of the comparison entity or -1 if no comparison entities are found
     */
    ...
    //#region - boolean
    /**
     * Processes all multipart expressions involving logical operators
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Gets the index of the next logical operator, accounting for precedence. Returns -1 if there are no logical operators.
     * @param {ExpressionNode[]} expressions 
     * @returns {Number} The index of the logical operator or -1 if no operators are found
     */

    //#endregion - boolean

    //#region - ternary and list comprehension
    /**
     * Processes all ternary expressions in a statement
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    /**
     * Processes all list comprehension expressions in a statement
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */

    //#endregion - ternary and list comprehension

    //#region - assignment
    /**
     * Processes all assignment expressions in a statement
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */
    ...
    /**
     * Gets the index of the next assignment / change operator. Returns -1 if no
     * such operator is found. Searches for operators from right to left
     * @param {ExpressionNode[]} expressions 
    ...
    //#region - returns
    /**
     * Processes statements beginning with a keyword
     * @param {ExpressionNode[]} expressions 
     * @returns {ExpressionNode[]}
     */
...
function _couldBeInHigherScope(block, name, firstUsageVar) {
  ...
  return false;
  // This is the first instance in this scope and there isn't already a global variable, and its assigned
...
function _processGroups(expressions) {
  var brackets = _classStaticPrivateMethodGet(this, StatementProcessor, _findAllMatchedBrackets).call(this, expressions);
  ...
  while (brackets.length > 0 && (oldBrackets === undefined || brackets.length !== oldBrackets.length)) {
    ...
    brackets = _classStaticPrivateMethodGet(this, StatementProcessor, _findAllMatchedBrackets).call(this, expressionTree);
    // Sort by left to right
    brackets.sort(function (a, b) {
      return a[0] - b[0];
    });
...
function _processParentheses(brackets, expressions) {
  ...
  // Something between the brackets
    ...
    var parts = this.split(betweenBrackets, _enums.ExpressionEntity.Comma);
    // tuple definition
    if (parts.length > 1) {
...
function _processSquareBrackets(brackets, expressions) {
  ...
  if (closeIndex > openIndex + 1) {
    betweenBrackets = expressions.slice(openIndex + 1, closeIndex);
    //parts = this.split(betweenBrackets, ExpressionEntity.Comma);
    parts = this.checkForListComp(betweenBrackets);
    // Add similar to function method call args
  }
  // list definition
  if (openIndex === 0 || parts.length === 0 || expressions[openIndex - 1].isOneOf([_enums.ExpressionCategory.ComparisonOperators, _enums.ExpressionCategory.LogicalOperators, _enums.ExpressionCategory.MathsOperators, _enums.ExpressionEntity.OpenParenthesis, _enums.ExpressionEntity.OpenSquareBracket, _enums.ExpressionEntity.OpenBrace, _enums.ExpressionEntity.Comma, _enums.ExpressionEntity.Colon, _enums.ExpressionEntity.Separator, _enums.ExpressionCategory.OtherKeywords, _enums.ExpressionEntity.ContinuationLine])) {
    ...
    var colonParts = this.split(betweenBrackets, _enums.ExpressionEntity.Colon);
    // slice / index / key
...
function _combineStringLiterals(expressions) {
  if (expressions.length < 2) return expressions;
  //expressions = expressions.filter(exp => !exp.is(ExpressionEntity.ContinuationLine));
  var i = 0;
  while (i < expressions.length - 1) {
...
function _findNextCalculationEntity(expressions) {
  ...
  var multDivide = [];
  var addSubtract = [];
  // Start from 1. First expression can't be an operator.
  for (var i = 0; i < expressions.length; i++) {
...
function _processBooleans(expressions) {
  ...
  while (nextIndex > -1) {
    ...
    expressions = start >= 0 ? expressions.slice(0, start).concat([boolNode], expressions.slice(nextIndex + 2)) : expressions.slice(0, nextIndex).concat([boolNode], expressions.slice(nextIndex + 2));
    // will keep selecting the same
