  ...
  _createClass(StatementBlock, [{
    ...
    value: function alwaysReturnsAValue() {
      ...
      if (_classPrivateFieldGet(this, _childBlocks).length === 0) {
        var lastStatements = this.getLastExecutedStatements();
        // Should only have 1 last statement...
...
function _checkStatementSymptoms2(block) {
  ...
  for (var i = 0; i < statements.length; i++) {
    ...
    // Infinite loop and unreachable due to while true
    ...
    for (_iterator15.s(); !(_step15 = _iterator15.n()).done;) {
      ...
      try {
        for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
          ...
          if ((0, _utils.containsExit)(exp)) return false;
...
function _checkSequentialIfs2(block) {
  ...
  for (var i = 0; i < statements.length - 1; i++) {
    ...
    // Gather all at once, or piece by piece?
...
function _createSequentialIf2(sequence) {
  ...
  //return SymptomFinder.createStatementSymptom(SymptomType.SequentialIfs, )
...
var ScopeBlock = /*#__PURE__*/function (_StatementBlock) {
  ...
  _createClass(ScopeBlock, [{
    ...
    value: function addUserDefinedClass(classType) {
      this.userDefinedClassMap.set(classType.name, classType);
      // TODO: search unconnectedFunctionCalls for functions matching the classname
    }
...
var ClassBlock = /*#__PURE__*/function (_ScopeBlock) {
  ...
  function ClassBlock(parentBlock, definitionStatement) {
    ...
    _classPrivateFieldSet(_assertThisInitialized(_this3), _classType, definitionStatement.getFirstExpression().getDataType());
    // CHANGE CLASS TRACKING TO USE DATATYPE
    ...
    //TODO: Class block symptoms this.addRules([this.#checkFunctionPrints]);
    return _this3;
  }
...
var FunctionBlock = /*#__PURE__*/function (_ScopeBlock2) {
  ...
  function FunctionBlock(parentBlock, definitionStatement) {
    ...
    _classPrivateFieldSet(_assertThisInitialized(_this4), _functionName, firstExp !== undefined ? firstExp.getTextValue() : "not parsed");
  ...
  _createClass(FunctionBlock, [{
    ...
    /**
     * Recursively checks statements for print and return
     * @param {Statement[]} statements 
     * @returns {Object[]} Each object contains the line number of the print statement, it's distance from the end of its branch in the function, and it's distance from the return in the branch if any
     */
